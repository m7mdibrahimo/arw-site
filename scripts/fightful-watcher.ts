...
function sanitizeWrestlingTerms(text: string): string {
  ...
  const cleaned = text
    ...
    .replace(arWord("(?:اي|إي|أى|أي)\\s*لي"), "إيه جيه لي")
    .replace(arWord("(?:اي|إي|أى|أي)\\s*(?:جاي|جي)\\s*لي"), "إيه جيه لي")
    .replace(/AJ\s*Styles/gi, "إيه جيه ستايلز")
    .replace(arWord("(?:اي|إي|أى|أي)\\s*ستايلز"), "إيه جيه ستايلز")
    .replace(arWord("(?:اي|إي|أى|أي)\\s*(?:جاي|جي)\\s*ستايلز"), "إيه جيه ستايلز")
    .replace(/CJ\s*Perry/gi, "سي جيه بيري")
    .replace(arWord("سي\\s*(?:جاي|جي)\\s*بيري"), "سي جيه بيري")
    .replace(/JD\s*McDonagh/gi, "جيه دي ماكدونا")
    .replace(/Dean\s*Malenko/gi, "دين مالينكو")
    .replace(/Orange\s*Cassidy/gi, "أورانج كاسيدي")
    ...
    // 6. Common wrestler name misspellings auto-correct
    .replace(arWord("ستيف\\s+رولينز"), "سيث رولينز")
    .replace(arWord("ستيف\\s+رولنز"), "سيث رولينز")

...
- **باقي عناصر العنوان (المصارعين، الأحداث، البطولات، الألقاب، الأفعال، التفاصيل)**:
  ...
  - **حظر الترجمة الحرفية الآلية الركيكة (Google Translate Style) وصياغة العنوان كصحفي بشري محترف (حاسمة جداً)**:
    ...
    - **قاعدة حاسمة لترجمة صيغة الشرط الافتراضي الإنجليزية "If [Subject] Could [Verb], We/They Would [Verb]"**:
      - هذه الصيغة تعبّر عن **استعداد/رغبة افتراضية** إذا سنحت الفرصة، وليست سؤالاً عن "القدرة" الجسدية أو المادية — **ممنوع منعاً باتاً ترجمة "Could" هنا إلى "استطاع" أو "قدر" وترجمة "We Would" إلى "فسوف نفعل" أو "سنقوم بذلك"** لأنها ترجمة حرفية ركيكة تُفقد الجملة معناها الطبيعي بالعربية.
      - ❌ العنوان الأصلي: "Mark Shapiro: If UFC Could Do Another White House Event Next Year, We Would" -> ❌ ترجمة ركيكة محظورة: "مارك شابيرو: إذا استطاع UFC إقامة حدث آخر في البيت الأبيض العام القادم فسوف نفعل"
      - ✅ **الصياغة الصحيحة الطبيعية**: "مارك شابيرو: إذا أتيحت لـUFC فرصة إقامة فعالية أخرى في البيت الأبيض العام المقبل، فسنفعل ذلك"
      - استخدم دائماً في هذه الصيغة تعبيرات طبيعية مثل: "إذا أتيحت له/لهم الفرصة.. فسيفعل ذلك"، "إذا سنحت الفرصة مجدداً.. لن يتردد"، "لو قُدّر له ذلك.. سيكررها".
...
export function tagInArabic(tag: string): string {
  ...
  if (ci && /[\u0600-\u06FF]/.test(ci)) return ci;
  // An English championship tag with no approved Arabic form («MLW World
  // Champion») is dropped (an empty tag is filtered as junk) rather than kept.
  if (/\b(?:Champion|Championship|Championships|Title|Titles)$/i.test(t)) return "";
  return t;
}
...
1. **قاعدة أسماء الاتحادات والعروض بالإنجليزية حصراً**:
   ...
   - **أسماء الألقاب والبطولات**: اكتب اسم البطولة بالعربية كما هو في القائمة المعتمدة المرسلة لك حرفيا (مثل «بطولة AEW العالمية»، «بطولة IWGP للوزن الخفيف»)، ولا تحوّل بطولة فردية (World Title) إلى بطولة فرق/زوجي (Tag Team) أو العكس.
...
2. **التنسيق المنظم والفصل بين السطور**:
   ...
   - **النتائج كما في المصدر حرفيا**: تقارير النتائج في المصدر غالبا سطر واحد لكل نزال («X def. Y»). لا تضف من عندك وصفا للمجريات أو الحركات أو السيطرة أو التدخلات — هذا اختلاق (حصل فعليا: تقرير ROH TV وصف كل نزال بـ«فرضت سيطرتها الكاملة» و«تدخلات سريعة من الطرفين» والمصدر لا يذكر شيئا من ذلك).
   - **الفقرات غير النزالية** المذكورة في المصدر (خطابات، مواجهات كلامية، هجمات، إعلانات) تُذكر أيضا في مكانها من العرض، بإيجاز ودقة.
   - اتبع هذا الترتيب الدقيق لكل مواجهة:
     ---
     **المواجهة [رقم المواجهة بالعربية]: [طبيعة المواجهة/اللقب إن وجد]**
     
     [جملة أو جملتان تنقلان ما ذكره المصدر عن هذه المواجهة فقط: الأطراف، اللقب إن وجد، وأي تفصيل مذكور في المصدر (تدخل، حركة الحسم، ما حدث بعدها). إذا كان المصدر يذكر النتيجة في سطر واحد فقط فاكتب جملة واحدة تذكر الطرفين واللقب — ممنوع اختراع مجريات أو حركات أو سيطرة أو تدخلات غير مذكورة]
   ...
   - في الحدث الرئيسي (Main Event):
     ...
     **الحدث الرئيسي (Main Event): [طبيعة النزال]**
     ...
     [ما ذكره المصدر عن الحدث الرئيسي فقط، بلا أي تفاصيل مخترعة]
...
${timing.isTonight || (timing.isPreview && !timing.isFuture) ? `     - 🚨 **تنبيه زمني حاسم جداً: هذا العرض يقام الليلة / اليوم (${arabicDate})!**
   ...
   - **حظر الترجمة الحرفية الآلية الركيكة وصياغة الأخبار كصحفي بشري محترف (Human Journalism vs Google Translate)**:
     ...
     - **قاعدة حاسمة لترجمة صيغة الشرط الافتراضي الإنجليزية "If [Subject] Could [Verb], We/They Would [Verb]"**: هذه الصيغة تعبّر عن استعداد/رغبة افتراضية إذا سنحت الفرصة، وليست سؤالاً عن "القدرة" — **ممنوع منعاً باتاً ترجمة "Could" إلى "استطاع" أو "قدر" وترجمة "We Would" إلى "فسوف نفعل"**:
       - ❌ "Mark Shapiro: If UFC Could Do Another White House Event Next Year, We Would" -> ❌ ترجمة ركيكة محظورة: "مارك شابيرو: إذا استطاع UFC إقامة حدث آخر في البيت الأبيض العام القادم فسوف نفعل"
       - -> ✅ **الصياغة الصحيحة**: "مارك شابيرو: إذا أتيحت لـUFC فرصة إقامة فعالية أخرى في البيت الأبيض العام المقبل، فسنفعل ذلك"
     - الصياغة يجب أن تكون بأسلوب محرر صحفي بشري رياضي محترف؛ فصيحة، رشيقة، مشوقة، ومفهومة جداً من القراءة الأولى.
...
4. **قاعدة أسماء الاتحادات والعروض بالإنجليزية حصراً (Strict Rule)**:
   ...
   - **أسماء المصارعين والمصارعات بالعربية دائماً وحصراً ودون أخطاء (ممنوع منعاً باتاً كتابة اسم أي مصارع بالإنجليزية سواء في العنوان أو المتن)**:
    ...
    - **قاعدة أسماء الألقاب والبطولات بالعربية الخالصة ودون تكرار اسم الاتحاد (Championship Titles)**:
      ...
      - **اسم البطولة كما في القائمة المعتمدة المرسلة لك حرفيا** (مثل «بطولة AEW العالمية»، «بطولة AEW العالمية للفرق»، «بطولة IWGP للوزن الخفيف»، «بطولة الولايات المتحدة»).
      - **لا تخلط بين الفردي والفرق أبدا**: World Title / World Championship = البطولة العالمية للفرديين؛ Tag Team = «للفرق» أو «للزوجي» فقط إذا قال المصدر Tag Team.

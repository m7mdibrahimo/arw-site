...
function crossPostToFacebookAndInstagram(key: string, item: { title: string; text?: string; fullText?: string; url: string; image?: string; kind?: string }) {
  ...
  // ── Shows now get BOTH:
  //    1. A regular image post (this server) → FB + IG as normal post
  //    2. A Reel + Story (show-reel-monitor.yml workflow) → FB Reel + FB Story + IG Reel + IG Story
  // Both systems are independent — no conflict, no duplication (different post types).
